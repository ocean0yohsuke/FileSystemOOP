...
class ObjectFileSystemFile extends ObjectFileSystemBase
{
	...
	function __construct(ObjectFileSystemFile $File)
	{
		ObjectFileSystemUtil::shadowPublicProps($File, $this);
	}
	
	function __call($objectname, $params)
	{
		if (!isset($this->FileList[$objectname]))	{
			throw new ObjectFileSystemException("Failed to call method '{$objectname}': wrong name '{$objectname}' was specified on {$this->namespace}.");
		}
		
		$return = call_user_func_array(array($this->FileList[$objectname], ObjectFileSystemConst::MAINMETHOD_NAME),
		                               $params);
		if (isset($return)) {
	...
	final function __ObjectFileSystemFile_construct($dirpath, $namespace, 
	                                                ObjectFileSystemGlobal $Global)
	{
	...
	final protected function __ObjectFileSystemFile_FileList($dirpath, $namespace)
	{
		...
		foreach( new DirectoryIterator($dirpath) as $iterator )
		{
			if ($iterator->isFile())	{
				...
				if ($this->Global->Config()->assertion() && !method_exists($classname, ObjectFileSystemConst::MAINMETHOD_NAME)) {
				...
				$this->haveInheritedOFSFile($classname);
		...
		private function haveInheritedOFSFile($classname)
		{
			...
			if (isset($Refl->getParentClass()->name)) {
				if ($Refl->getParentClass()->name == 'ObjectFileSystemFile') {
					...
					$this->haveInheritedOFSFile($Refl->getParentClass()->name);
				}
			} else {					
...
class ObjectFileSystemException extends Exception
{
	function getException()
	{
		...
		else
		{
			...
			{
				if (!preg_match('/^#[0-9]+ .*\\ObjectFileSystem\.php\([0-9]+\): /i', $value))
				{
